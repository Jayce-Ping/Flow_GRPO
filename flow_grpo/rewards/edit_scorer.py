...
class EditScorer:
    ...
    def __call__(self, images: List[Image.Image], prompts: List[str], metadata : List[dict]) -> np.ndarray:
        ...
        def check_metadata_keys(prompt, meta):
            necessary_keys = ['layout', 'editing_prompts']
    ...
    async def _score_all_async(self, images: List[Image.Image], prompts: List[str], metadata: List[dict]):
        """异步批量评分所有项目"""
        tasks = [self.score_one_item_async(image, prompt, meta) 
    ...
    async def score_one_item_async(self, image, prompt, metadata):
        layout = metadata.get('layout', '1x1').split('x')
        layout = (int(layout[0]), int(layout[1]))
        editing_prompts = metadata['editing_prompts']
        main_prompt = editing_prompts[0]
        editing_prompts = editing_prompts[1:]
        sub_images = divide_image(image, layout)
        
        # 并行执行所有 SC 评分
        SC_tasks = []
        for img1, img2, edit_prompt in zip(sub_images[:-1], sub_images[1:], editing_prompts):
