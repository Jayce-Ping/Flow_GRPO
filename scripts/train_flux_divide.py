...
These two numbers should be equal
    ...
    while True:
        ...
        for i in tqdm(
            ...
            if all(h == heights[0] for h in heights) and all(w == widths[0] for w in widths) and all(l == layouts[0] for l in layouts):
                ...
                with autocast():
                    ...
                    # all_log_probs: List[Tensor(batch_size)] with length num_train_steps -> List[Tensor(num_train_steps) with length batch_size
                ...
                # all_log_probs: List[Tensor(num_train_steps)] with length batch_size
        ...
        if config.per_prompt_stat_tracking:
            ...
            advantages = stat_tracker.compute_advantages(prompts, gathered_rewards['avg'])
            ...
            for i, sample in tqdm(
                ...
                for j in tqdm(
                    ...
                    with accelerator.accumulate(transformer):
                        ...
                        if config.train.beta > 0:
                            if config.sample.cps:
                                kl_loss = ((prev_sample_mean - prev_sample_mean_ref) ** 2).mean(dim=tuple(range(1, prev_sample_mean.ndim)), keepdim=True)
                            else:
                                kl_loss = ((prev_sample_mean - prev_sample_mean_ref) ** 2).mean(dim=tuple(range(1, prev_sample_mean.ndim)), keepdim=True) / (2 * std_dev_t ** 2)
