...
def main(_):
    ...
    while True:
        ...
        for i in tqdm(
            ...
            timesteps = torch.as_tensor(config.train.timesteps, device=accelerator.device, dtype=torch.long)
                ...
                [
                    {
                        ...
                        'timesteps': pipeline.scheduler.timesteps[timesteps].unsqueeze(0).cpu(),
                        'next_timesteps': pipeline.scheduler.timesteps[timesteps + 1].unsqueeze(0).cpu(),
            ...
            for i, sample in tqdm(
                ...
                for j in tqdm(
                    ...
                    with accelerator.accumulate(transformer):
                        ...
                        dt = next_t_expanded - t_expanded # (batch_size, 1, 1, 1)
                        ...
                        old_next_xt_pred = (xt - dt * old_v_pred).detach()
                        new_next_xt_pred = xt - dt * new_v_pred
                        ref_next_xt_pred = (xt - dt * v_ref).detach()

                        log_prob = -((new_next_xt_pred - ref_next_xt_pred) ** 2).mean(dim=tuple(range(1, x0.ndim)), keepdim=True)
                        old_log_prob = -((old_next_xt_pred - ref_next_xt_pred) ** 2).mean(dim=tuple(range(1, x0.ndim)), keepdim=True)
                        ...
                        kl_loss = ((new_v_pred - v_ref) ** 2).mean(dim=tuple(range(1, ref_next_xt_pred.ndim)), keepdim=True)
                        ...
                        # Track loss tensors
                        if config.enable_mem_log:
                            memory_profiler.track_tensors(training_tensors, "loss_terms")
                            if i % 10 == 0:
