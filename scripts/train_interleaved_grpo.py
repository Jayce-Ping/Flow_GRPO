...
def sample_text_with_logprobs(
    ...
    while step < max_length:
        packed_text_embedding = model.language_model.get_input_embeddings(curr_tokens)
        ...
        if do_sample and temperature > 0:
            probs = torch.exp(log_probs_all) # (B, vocab)
            next_token = torch.multinomial(probs, num_samples=1).squeeze(1) # (B,)
        else:
            next_token = logits.argmax(dim=-1) # (B,)

        next_token = next_token.view(-1) # (B,)
        lp = log_probs_all.gather(-1, next_token.unsqueeze(-1)).squeeze(-1) # (B, )
        token_log_probs.append(lp)
        generated_ids.append(next_token)

        key_values_lens = key_values_lens + 1
        # The following code works only when batch_size=1
...
def compute_text_grpo_loss(
    ...
    packed_text_embedding = model.language_model.get_input_embeddings(input_tokens)
    ...
    if ref_model is not None and beta > 0:
        with torch.no_grad():
            ref_embedding = ref_model.get_input_embeddings(input_tokens)
...
def sample_interleaved_trajectory(
    ...
    with torch.autocast(device_type="cuda", enabled=True, dtype=torch.bfloat16):
        ...
        img_result = inferencer.gen_image(
            ...
            cfg_text_scale=inference_hyper.get("cfg_text_scale", 1.0),
...
def train_interleaved_step(
    ...
    if len(image_data["all_latents"]) > 0:
        ...
        all_latents = image_data["all_latents"]
        log_probs = image_data["all_log_probs"]
        timesteps = image_data["timesteps"]

        # Build sample dict matching existing flow_grpo format
        flow_sample = {
            "latents": all_latents[:, :-1] if len(all_latents) > 1 else all_latents,
            "prev_latents": all_latents[:, 1:] if len(all_latents) > 1 else all_latents,
            "log_probs": log_probs,
            "timesteps": timesteps,
...
def main(_):
    ...
    vit_config.num_hidden_layers = vit_config.num_hidden_layers - 1  # match inference convention
    ...
    if model.config.visual_gen:
        ...
        model.vae2llm = model.vae2llm.to(accelerator.device)
        model.llm2vae = model.llm2vae.to(accelerator.device)
        model.latent_pos_embed = model.latent_pos_embed.to(accelerator.device)

    if model.config.visual_und:
        model.vit_model = model.vit_model.to(accelerator.device)
        model.connector = model.connector.to(accelerator.device)
        model.vit_pos_embed = model.vit_pos_embed.to(accelerator.device)
