...
def grid_consistency_clip_nft_ode():
    gpu_number = 4
    config = compressibility()
    ...
    config.train.timestep_fraction = 0.99
    config.train.guidance_scale = 3.5
    config.train.timesteps = list(range(10))
    config.train.beta = 0
    config.train.nft_beta = 1
    config.train.decay_type = 1
    ...
    config.save_dir = os.path.join(SAVE_DIR, f'grid-consistency-subclip', f'flux-{gpu_number}gpu-2by2-half_grid-times-consistency-plus-clipT_10steps-ode')
